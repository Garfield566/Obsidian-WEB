...
class EmergentTagDetector:
    ...
    def _detect_entity_suggestions(
        self,
        cluster_notes: list,
        combined_text: str,
    ) -> list[EmergentTagSuggestion]:
        """Détecte les entités (noms, dates, lieux) et génère des suggestions.

        Cette méthode analyse les notes pour détecter:
        1. Noms de personnes (auteurs connus + pattern Prénom Nom)
        2. Dates significatives (années, siècles)
        3. Lieux géographiques

        Args:
            cluster_notes: Notes du cluster à analyser
            combined_text: Texte combiné des notes

        Returns:
            Liste de suggestions de tags pour les entités détectées
        """
        suggestions = []
        note_paths = [note.path for note in cluster_notes]

        # Compteurs pour agréger les détections par note
        person_counts: dict[str, dict] = {}  # {nom: {"count": n, "notes": [paths]}}
        date_counts: dict[str, dict] = {}    # {date: {"count": n, "notes": [paths]}}
        place_counts: dict[str, dict] = {}   # {lieu: {"count": n, "notes": [paths]}}

        # Liste de lieux connus (capitales, grandes villes, régions)
        KNOWN_PLACES = {
            # Capitales européennes
            "paris", "londres", "berlin", "rome", "madrid", "vienne", "athènes",
            "amsterdam", "bruxelles", "lisbonne", "varsovie", "prague", "budapest",
            "stockholm", "copenhague", "oslo", "helsinki", "dublin", "moscou",
            # Grandes villes
            "marseille", "lyon", "bordeaux", "toulouse", "strasbourg", "nantes",
            "new york", "los angeles", "chicago", "boston", "washington",
            "pékin", "shanghai", "tokyo", "kyoto", "delhi", "mumbai",
            # Régions et pays
            "france", "allemagne", "italie", "espagne", "angleterre", "russie",
            "grèce", "égypte", "chine", "japon", "inde", "états-unis", "amérique",
            "europe", "asie", "afrique", "méditerranée",
            # Lieux historiques
            "carthage", "sparte", "corinthe", "thèbes", "alexandrie", "babylone",
            "jérusalem", "constantinople", "byzance", "troie", "pompéi",
        }
        ...
        for note in cluster_notes:
            ...
            # === 1. DÉTECTION DES NOMS DE PERSONNES ===

            # Auteurs/philosophes connus
            for author in KNOWN_AUTHORS:
                if author in note_text_lower:
                    # Vérifie la proéminence (doit être un sujet principal)
                    prominence = self._calculate_entity_prominence(
                        author, note_text_lower, note_title_lower
                    )
                    if prominence >= self.MIN_ENTITY_PROMINENCE:
                        if author not in person_counts:
                            person_counts[author] = {"count": 0, "notes": [], "prominence": 0}
                        person_counts[author]["count"] += 1
                        person_counts[author]["notes"].append(note.path)
                        person_counts[author]["prominence"] = max(
                            person_counts[author]["prominence"], prominence
                        )

            # Pattern Prénom Nom (pour les personnes non connues)
            person_matches = self.PATTERNS["person_pattern"].findall(note_text)
            for first, last in person_matches:
                full_name = f"{first} {last}".lower()
                # Évite les faux positifs (mots courants avec majuscules)
                if full_name in self.STOP_WORDS:
                    continue
                if any(w in full_name for w in ["le ", "la ", "les ", "du ", "de "]):
                    continue

                prominence = self._calculate_entity_prominence(
                    full_name, note_text_lower, note_title_lower
                )
                if prominence >= self.MIN_ENTITY_PROMINENCE:
                    if full_name not in person_counts:
                        person_counts[full_name] = {"count": 0, "notes": [], "prominence": 0}
                    person_counts[full_name]["count"] += 1
                    if note.path not in person_counts[full_name]["notes"]:
                        person_counts[full_name]["notes"].append(note.path)
                    person_counts[full_name]["prominence"] = max(
                        person_counts[full_name]["prominence"], prominence
                    )

            # === 2. DÉTECTION DES DATES ===

            # Années (1000-2100)
            year_pattern = re.compile(r'\b(1[0-9]{3}|20[0-2][0-9])\b')
            year_matches = year_pattern.findall(note_text)
            for year in year_matches:
                year_int = int(year)
                # Filtre les années trop génériques (ex: 2000, 1000)
                if year_int % 100 == 0:
                    continue
                # Vérifie que l'année apparaît plusieurs fois ou dans le titre
                occurrences = note_text.count(year)
                if occurrences >= 2 or year in note.title:
                    if year not in date_counts:
                        date_counts[year] = {"count": 0, "notes": [], "type": "year"}
                    date_counts[year]["count"] += occurrences
                    if note.path not in date_counts[year]["notes"]:
                        date_counts[year]["notes"].append(note.path)

            # Siècles (chiffres romains)
            for numeral in ROMAN_NUMERALS:
                pattern = re.compile(rf'\b{numeral}e?\s*siècle\b', re.IGNORECASE)
                if pattern.search(note_text):
                    century_key = f"{numeral}e siècle"
                    if century_key not in date_counts:
                        date_counts[century_key] = {"count": 0, "notes": [], "type": "century"}
                    date_counts[century_key]["count"] += 1
                    if note.path not in date_counts[century_key]["notes"]:
                        date_counts[century_key]["notes"].append(note.path)

            # === 3. DÉTECTION DES LIEUX ===

            for place in KNOWN_PLACES:
                if place in note_text_lower:
                    # Vérifie que c'est bien un lieu (pas juste une mention)
                    prominence = self._calculate_entity_prominence(
                        place, note_text_lower, note_title_lower
                    )
                    if prominence >= self.MIN_ENTITY_PROMINENCE * 0.8:  # Seuil légèrement plus bas
                        if place not in place_counts:
                            place_counts[place] = {"count": 0, "notes": [], "prominence": 0}
                        place_counts[place]["count"] += 1
                        if note.path not in place_counts[place]["notes"]:
                            place_counts[place]["notes"].append(note.path)
                        place_counts[place]["prominence"] = max(
                            place_counts[place]["prominence"], prominence
                        )

        # === GÉNÉRATION DES SUGGESTIONS ===

        # Suggestions pour les personnes
        for person, data in person_counts.items():
            if len(data["notes"]) < self.MIN_NOTES_FOR_SUGGESTION:
                continue

            # Normalise le nom pour le tag
            tag_name = self._normalize_for_tag(person)

            # Vérifie que le tag n'existe pas déjà
            if self._normalize(tag_name) in self._existing_tags_normalized:
                continue

            confidence = min(0.85, 0.65 + data["prominence"] * 0.20)
            ...
            suggestions.append(EmergentTagSuggestion(
                ...
                notes=data["notes"],
                source_terms=[person],
                reasoning=f"Personne '{person}' détectée dans {len(data['notes'])} notes (proéminence: {data['prominence']:.0%})",
                metadata={
        ...
        # Suggestions pour les dates
        for date_str, data in date_counts.items():
            if len(data["notes"]) < self.MIN_NOTES_FOR_SUGGESTION:
                continue

            # Détermine le format du tag selon le type
            if data["type"] == "century":
                # Extrait le chiffre romain (ex: "XIXe siècle" -> "XIX")
                tag_name = date_str.split("e")[0].strip().upper()
            else:
                # Année simple
                tag_name = date_str
            ...
            confidence = min(0.80, 0.60 + (len(data["notes"]) / 10) * 0.20)
            ...
            suggestions.append(EmergentTagSuggestion(
                ...
                confidence=confidence,
                notes=data["notes"],
                source_terms=[date_str],
                reasoning=f"Date '{date_str}' détectée dans {len(data['notes'])} notes",
                metadata={
        ...
        # Suggestions pour les lieux
        for place, data in place_counts.items():
            if len(data["notes"]) < self.MIN_NOTES_FOR_SUGGESTION:
                continue

            # Format du tag: geo\lieu
            tag_name = f"geo\\{self._normalize_for_tag(place)}"

            # Vérifie que le tag n'existe pas déjà
            if self._normalize(tag_name) in self._existing_tags_normalized:
                continue

            confidence = min(0.80, 0.60 + data["prominence"] * 0.20)
            ...
            suggestions.append(EmergentTagSuggestion(
                ...
                notes=data["notes"],
                source_terms=[place],
                reasoning=f"Lieu '{place}' détecté dans {len(data['notes'])} notes (proéminence: {data['prominence']:.0%})",
                metadata={
                    "category": "entity_place",
                    "occurrences": data["count"],
                    "prominence": data["prominence"],
                },
            ))
