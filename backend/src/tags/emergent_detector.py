...
class EmergentTagDetector:
    ...
    def _detect_domain_vocabulary_suggestions(
        ...
        print(f"      🔎 _detect_domain_vocabulary_suggestions: {len(cluster_notes)} notes, texte={len(text_lower)} chars, {len(self.HIERARCHY)} domaines racine", flush=True)
        ...
        for domain_name, domain_data in self.HIERARCHY.items():
            ...
            if not is_valid:
                if len(vsc_found) + len(vsca_found) > 0:
                    print(f"      ❌ SEUIL NON ATTEINT: {domain_name} ({len(vsc_found)} VSC + {len(vsca_found)} VSCA) - {validation_reason}", flush=True)
                continue

            print(f"      ✅ DOMAINE VALIDE: {domain_name} ({len(vsc_found)} VSC + {len(vsca_found)} VSCA)", flush=True)
            ...
            if self._normalize(domain_tag) in self._existing_tags_normalized:
                print(f"      ⏭️  SKIP (tag existant): {domain_name}", flush=True)
                continue
        ...
        print(f"      📊 PHASE 2: {len(domain_matches)} domaines validés avant filtrage weak", flush=True)
        for m in domain_matches[:10]:
            print(f"         {m['name']:30s} score={m['total_vocab']:3d} weak={m['is_weak']}", flush=True)
        if len(domain_matches) > 10:
            print(f"         ... et {len(domain_matches) - 10} autres", flush=True)
        ...
        for match in domain_matches:
            ...
            if is_weak:
                ...
                if total_vocab != best_score:
                    ...
                    print(f"      ❌ WEAK FILTRÉ (pas dominant): {domain_name} score={total_vocab} vs best={best_score}", flush=True)
                    continue
                if second_best_score > 0 and total_vocab < second_best_score * self.WEAK_DOMAIN_DOMINANCE_RATIO:
                    ...
                    print(f"      ❌ WEAK FILTRÉ (ratio): {domain_name} score={total_vocab} vs 2nd={second_best_score} (ratio={self.WEAK_DOMAIN_DOMINANCE_RATIO})", flush=True)
                    continue
