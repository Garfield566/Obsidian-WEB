"""
Extraction PC1 Phase 2 - 4 domaines restants (1/4 de la charge).
...
# Configuration des domaines RESTANTS pour PC1 (Phase 2)
PC1_PHASE2_DOMAINS = {
    "sciences_sociales": {
        "label": "Sciences Sociales",
        "domains": {
            "geographie": DOMAINS_BY_CATEGORY["sciences_sociales"]["domains"]["geographie"],
            "philosophie": DOMAINS_BY_CATEGORY["sciences_sociales"]["domains"]["philosophie"],
            "droit": DOMAINS_BY_CATEGORY["sciences_sociales"]["domains"]["droit"],
        }
    },
    "sciences_humaines": {
        "label": "Sciences Humaines",
        "domains": {
            "psychologie": DOMAINS_BY_CATEGORY["sciences_humaines"]["domains"]["psychologie"],
        }
    }
}
...
def draw_global_progress(current, total, domain_name, category_label, elapsed_total):
    ...
    print("\n" + "=" * 80)
    print(f"PC1 PHASE 2 - PROGRESSION: [{bar}] {percent:.1f}%")
...
def main():
    print("=" * 80)
    print("EXTRACTION PC1 PHASE 2 - DOMAINES RESTANTS (4)")
    print("=" * 80)

    total_domains = count_domains(PC1_PHASE2_DOMAINS)

    print(f"\nTotal domaines Phase 2 PC1: {total_domains}")
    print(f"\nCategories:")
    for cat_key, cat_data in PC1_PHASE2_DOMAINS.items():
    ...
    for category_key, category_data in PC1_PHASE2_DOMAINS.items():
    ...
    # Resume final
    print("\n" + "=" * 80)
    print("RESUME FINAL PC1 PHASE 2")
    print("=" * 80)
    ...
    if completed_domains:
        ...
        print(f"\n  {'TOTAL PC1 PHASE 2':<22} {total_vocab:>6} vocab, {total_specialized:>5} specialises")
    ...
    hours = int(elapsed_total / 3600)
    mins = int((elapsed_total % 3600) / 60)
    print(f"\nTemps total PC1 Phase 2: {hours}h{mins:02d}min")

    # Prochaines etapes
    if len(completed_domains) == total_domains:
        print("\n" + "=" * 80)
        print("[OK] EXTRACTION PC1 PHASE 2 TERMINEE!")
        print("=" * 80)
        print("\nAttendre que PC2 Phase 2 termine, puis fusionner:")
