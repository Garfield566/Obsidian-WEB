"""
Extraction PC2 - 21 domaines (Arts + Gastronomie + Religions + Littérature).
À exécuter sur le deuxième PC.
"""
...
PC2_DOMAINS = {
    "arts_visuels": DOMAINS_BY_CATEGORY["arts_visuels"],
    "arts_audiovisuels": DOMAINS_BY_CATEGORY["arts_audiovisuels"],
    "gastronomie": DOMAINS_BY_CATEGORY["gastronomie"],
    "religions": DOMAINS_BY_CATEGORY["religions"],
    "litterature": DOMAINS_BY_CATEGORY["litterature"],
}
...
def draw_global_progress(current, total, domain_name, category_label, elapsed_total):
    ...
    print("\n" + "=" * 80)
    print(f"PC2 - PROGRESSION: [{bar}] {percent:.1f}%")
...
def main():
    print("=" * 80)
    print("EXTRACTION PC2 - ARTS + GASTRONOMIE + RELIGIONS + LITTERATURE (21 domaines)")
    print("=" * 80)

    total_domains = count_domains(PC2_DOMAINS)

    print(f"\nTotal domaines PC2: {total_domains}")
    print(f"\nCategories:")
    for cat_key, cat_data in PC2_DOMAINS.items():
        domain_count = len(cat_data["domains"])
        print(f"  - {cat_data['label']}: {domain_count} domaines")

    print(f"\nRepertoire de sortie: {OUTPUT_DIR}")
    print(f"\n[TIME] Temps estime: {total_domains * 1.5:.0f}h - {total_domains * 2.5:.0f}h")
    print("         (~32-52 heures pour PC2)")

    # Demander confirmation
    print("\n" + "=" * 80)
    print("ATTENTION: Extraction longue (~1.5-2 jours)")
    print("=" * 80)
    response = input("\nLancer l'extraction PC2? (o/n): ")
    ...
    for category_key, category_data in PC2_DOMAINS.items():
    ...
    if completed_domains:
        ...
        print(f"\n  {'TOTAL PC2':<22} {total_vocab:>6} vocab, {total_specialized:>5} specialises")
    ...
    hours = int(elapsed_total / 3600)
    mins = int((elapsed_total % 3600) / 60)
    print(f"\nTemps total PC2: {hours}h{mins:02d}min")

    # Prochaines etapes
    if len(completed_domains) == total_domains:
        print("\n" + "=" * 80)
        print("[OK] EXTRACTION PC2 TERMINEE!")
        print("=" * 80)
        print("\nTransferer les resultats vers PC1:")
        print("  1. Copier le dossier 'extracted_by_domain/' vers PC1")
        print("  2. Sur PC1, fusionner avec: python extract_by_domain.py --merge-only")
