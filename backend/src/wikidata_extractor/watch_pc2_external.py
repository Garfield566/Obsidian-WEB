"""
Monitoring de l'extraction PC2 (21 domaines) sur disque externe F:\
"""
...
def monitor_pc2(refresh_seconds=5):
    """Surveille la progression PC2."""

    all_domains = get_all_domains_flat(PC2_DOMAINS)
    total_domains = len(all_domains)

    start_time = time.time()

    print("=" * 80)
    print(f"MONITORING PC2 - {total_domains} DOMAINES (Disque E:\\)")
    print("=" * 80)
    print(f"Repertoire: {OUTPUT_DIR_EXTERNAL}")
    ...
    try:
        while True:
            ...
            print(f"PC2: [{bar}] {global_percent:.1f}% | ", end='')
            ...
            if completed_count == total_domains:
                ...
                print("=" * 80)
                print("\nPROCHAINE ETAPE:")
                print(f"  1. Ejecter disque E:\\ de PC2")
                print(f"  2. Rebrancher E:\\ sur PC1")
                print(f"  3. Sur PC1, copier: {OUTPUT_DIR_EXTERNAL}")
                print(f"     Vers: C:\\Users\\robin tual\\quartz\\backend\\src\\wikidata_extractor\\extracted_by_domain\\")
                print(f"  4. Fusionner: python extract_by_domain.py --merge-only")
